...
char* codToken[] = {"COMMA","SEMICOLON","LPAR","RPAR","LBRACKET","RBRACKET","LACC","RACC","ADD","SUB","MUL","DIV","DOT","AND","OR","NOT","NOTEQ","ASSIGN","EQUAL","LESS","LESSEQ","GREATER","GREATEREQ","CT_INT","CT_REAL","CT_STRING","CT_CHAR","END","BREAK","CHAR","DOUBLE","ELSE","FOR","IF","INT","RETURN","STRUCT","VOID","WHILE","ID"};
...
void tkerr(const Token *tk,const char *fmt,...){
    va_list va;
    va_start(va,fmt);
    fprintf(stderr,"Eroare la linia %d: ",tk->line);
...
int consume(int cod){
    printf("@consume %s / %s\n",codToken[cod],codToken[crtTk->code]);
	if(crtTk->code == cod){
		consumedTk = crtTk;
		crtTk = crtTk->next;
		return 1;
				...
				if(consume(RPAR)){
					if(stm()){
						if(consume(ELSE)){
							...
							}else tkerr(crtTk,"lipseste statementul lui ELSE");
						}
					return 1;
					}
				}
        ...
        if(consume(ID)){
            ...
            while(1){
                if(consume(COMMA)){
                    if(consume(ID)){
                        arrayDecl();
                    }else tkerr(crtTk,"lipseste ID-ul variabilei");
                }
                else{
                    break;
                }
